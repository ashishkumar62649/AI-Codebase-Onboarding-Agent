...
from fcode.contracts import FCodeConfig, FileType, RepoInput, ScanResult, ScannedFile, SkippedFileDiagnostic
...
MAX_ELIGIBLE_CONTENT = 52_428_800  # 50 MiB
...
def scan_repository(repo: RepoInput, config: FCodeConfig) -> ScanResult:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root):
        ...
        for filename in filenames:
            ...
            skipped_diag, size = _skip_file_if_unsuitable(full_path)
            if skipped_diag:
            ...
            safe_content, _has_secrets = detect_secrets(text)
            ...
            eligible.append(
                ScannedFile(
                    file_path=rel_path,
                    file_type=classify_file_type(full_path) if not is_binary else FileType.DOC,
                    size_bytes=len(raw),
                    is_binary=is_binary,
...
def _skip_file_if_unsuitable(path: str) -> tuple[SkippedFileDiagnostic | None, int]:
    try:
