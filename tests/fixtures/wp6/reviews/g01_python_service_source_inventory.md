...
### 10.5 Graph Node Total: 22

7 (file) + 2 (route) + 9 (symbol) + 4 (import) = 22.

**DIVERGENCE NOTE:** The existing manifest states `graph_nodes: 18`. The source-derived count is 22. The discrepancy of 4 likely stems from the manifest being generated by a pipeline run that may have excluded error-status files from graph node creation, or used a different parser version. Per the current production code in `build_graph()`, every `ParsedFile` (including error-status files) produces a file node. This review records the code-derived truth.

---

## 11. Graph-Edge Inventory

### 11.1 Defines Edges (10)
...
**DIVERGENCE NOTE:** The existing manifest states `graph_edges: 19`. The source-derived count is 21. The discrepancy of 2 likely stems from the manifest not counting the 2 handler→route `defines` edges (which are a less obvious edge type). This review records the code-derived truth.
