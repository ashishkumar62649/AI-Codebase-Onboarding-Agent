"""DeepOrra CLI — Typer application.

Registers all commands from deeporra.cli.commands modules.
...
from deeporra.contracts import IndexServiceProtocol, StatusServiceProtocol

app = typer.Typer(
    name="DeepOrra",
    help="DeepOrra — local-first repository intelligence tool for AI coding agents.",
...
def configure_app(
    *,
    ...
    if index_service is not None:
        from deeporra.cli.commands.index_cmd import set_index_service

        set_index_service(index_service)
    if status_service is not None:
        from deeporra.cli.commands.status_cmd import set_status_service
