...
from deeporra.scanner.ignore_rules import IgnoreRules, HARDCODED_IGNORED_DIRS
from deeporra.scanner.secret_detector import detect_secrets
...
def scan(repo: RepoInput, config: DeepOrraConfig) -> ScanResult:
...
def scan_repository(repo: RepoInput, config: DeepOrraConfig) -> ScanResult:
            ...
            if ignore_rules.is_ignored(full_path):
                skipped.append(
                    SkippedFileDiagnostic(
                        ...
                        details="Matched .gitignore or .deeporraignore",
                    )
                )
                continue
