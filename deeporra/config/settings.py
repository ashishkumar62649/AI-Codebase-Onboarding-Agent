...
CONFIG_FILE_NAME = ".deeporra/config.json"
...
def load_config(repo_path: str) -> DeepOrraConfig:
    ...
    return DeepOrraConfig(
        repo_path=data.get("repo_path", repo_path),
        db_path=os.path.join(
            repo_path, storage.get("sqlite_path", ".deeporra/index.db")
        ),
        chroma_path=os.path.join(
            repo_path, storage.get("chroma_path", ".deeporra/chroma")
        ),
...
def _build_config_json(config: DeepOrraConfig) -> dict:
    return {
...
def save_config(repo_path: str, config: DeepOrraConfig) -> None:
...
def create_default_config(repo_path: str, force: bool = False) -> DeepOrraConfig:
    path = _config_path(repo_path)
    if path.exists() and not force:
        raise FileExistsError(f"Config file already exists: {path}")
    config = DeepOrraConfig(repo_path=repo_path)
