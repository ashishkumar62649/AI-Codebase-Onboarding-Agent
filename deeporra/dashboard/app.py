...
st.set_page_config(
    page_title="DeepOrra Dashboard",
    page_icon="\u2139\ufe0f",
    layout="wide",
    initial_sidebar_state="expanded",
)

st.markdown("## DeepOrra Dashboard \u2014 Prototype")
st.markdown(
...
def _connect_repo(path: str) -> str | None:
    ...
    try:
        ...
        return f"Repository at {p} is not indexed. Run `DeepOrra index` first."
...
if qs is None:
    st.info(
        ...
        "The repository must already be indexed with `DeepOrra index`."
    )
    st.stop()
